...
# PYTHON3.6 - hopefully or Python2.7
# 
# Author: Kate Willett
...
# -----------------------
# This code calculates regional average timeseries for the value and all uncertainty sources in addition to the coverage
# uncertainty based on ERA-Interim (soon to be ERA-5)
#
# This is 99% Robert Dunn's old code for the land (hadisdh_error_calculations.py in /data/local/hadkw/HADCRUH2/UPDATE2017/PROGS/HADISDH_BUILD/ 
# 
# -----------------------
# LIST OF MODULES
# -----------------------
...
# DATA
# -----------------------
# The HadISDH marine file with all values and uncertainty components in - one variable at a time
# /data/local/hadkw/HADCRUH2/MARINE/DATA/HadISDH.marine<var>.<version>_BClocal<SHIP>5x5<timings>_anoms8110_<NowMon><NowYear>_cf.nc
#
# The ERA-Interim file
# /data/local/hadkw/HADCRUH2/UPDATE2017/OTHERDATA/<var>2m_monthly_5by5_ERA-Interim_data_19792017_anoms1981-2010.nc
#
# A file to judge gridboxes that are marine
# /data/local/hadkw/HADCRUH2/UPDATE2017/OTHERDATA/HadCRUT.4.3.0.0.land_fraction.nc
# 
# -----------------------
# HOW TO RUN THE CODE
# -----------------------
#
# Make sure the dates, filepaths and versions are correct within this file.
#
# module load scitools/experimental-current
# python
#
# hadisdh_error_calculations_marine.py
# 
# -----------------------
# OUTPUT
# -----------------------
# A set of ascii files for eahc region (globe, Nhem, Tropics, Shem) withe time series for the anomaly variable and all uncertainty components combined and seperately
#
# 
# -----------------------
...
# RJHD routines
YEAREND = '2018'
DATALOCATION = "/data/local/hadkw/HADCRUH2/MARINE/DATA/GRIDS/"
OUTDATALOCATION = "/data/local/hadkw/HADCRUH2/MARINE/DATA/TIMESERIES/"
OTHERDATALOCATION = "/data/local/hadkw/HADCRUH2/UPDATE"+YEAREND+"/OTHERDATA/"
HADISDH_VER = "1.0.0.2019f"
HADISDH_DATE = "FEB2019"

###########################################################
# Set up a Region object
class Region(object):
...
#*******************************
def simple_outfile_write(filename, times, ts, sample, coverage, obs, combined):
    '''
    ...
    for d in range(len(ts)):
        ...
        outfile.write("".join(["{:10.5f} "*6,"\n"]).format(times[d], ts[d], sample[d], coverage[d], obs[d], combined[d]))
 
    outfile.write("".join(["{:10s} "*6,"\n"]).format('#month','hadisdh','sample','coverage','obs','combined'))
...
#*******************************
def full_outfile_write(filename, times, ts, sample, coverage, obs, combined):
    '''
    ...
    for d in range(len(ts)):
        ...
        outfile.write("".join(["{:10.5f} "*10,"\n"]).format(times[d], ts[d], ts[d]-sample[d], ts[d]+sample[d], ts[d]-coverage[d], ts[d]+coverage[d], ts[d]-obs[d], ts[d]+obs[d], ts[d]-combined[d], ts[d]+combined[d]))

    outfile.write("".join(["{:10s} "*10,"\n"]).format('#month','hadisdh','sample-','sample+','coverage-','coverage+','obs-','obs+','combined-','combined+'))
    outfile.write("# all 2-sigma errors\n")
    outfile.write("# 1981-2010 anomaly period")
    outfile.close()
