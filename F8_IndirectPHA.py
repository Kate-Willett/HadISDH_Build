...
# STATLIST='/LISTS_DOCS/goodforHadISDH.'+version+'_IDPHAall.txt'	# removed all 'bad' DPD and T stations (6)
# List of WMO ID for the stations removed from Td during PHA
# TDBADSFILE='/pha52jgo/data/hadisdh/td/corr/badlist.txt'
#
# Break locations are taken from pha output for T and DPD
# TBREAKFIL='LISTS_DOCS/HadISDH.landT.'+version+'_PHA.log'
# DPDBREAKFIL='LISTS_DOCS/HadISDH.landDPD.'+version+'_PHA.log'
#
# Correlation matrix of each station with 40 highest correlating neighbours from PHA
# CORRFIL='/pha52jgo/data/hadisdh/rh/corr/corr.log'
# Raw monthly mean data in ASCII 
# INRAW='/MONTHLIES/ASCII/RHABS/'
# 
# -----------------------
# HOW TO RUN THE CODE
# -----------------------
...
# python IndirectPHA_JAN2015.py --var q --runtype all
# var can be q, rh, e, t, tw
# runtype can be all or 999999 WMO-like Station ID
# 
# -----------------------
# OUTPUT
# -----------------------
...
# OUTHOM='/MONTHLIES/HOMOG/IDPHAASCII/RHDIR/'
# Log file of changepoint locations, adjustment magnitude and 1SD uncertainty actual and acumulated
# BREAKSINFO='/LISTS_DOCS/HadISDH.landRH.'+version+'_IDPHA.log'
# A list of stations for which there are fewer than 7 neighbours that correlate greater than 0.0
# NONEIGHBOURSLIST='/LISTS_DOCS/noneighbours_landRH.'+version+'_IDPHA.txt'
# Good stations list to carry on for further processing
# GOTNEIGHBOURSLIST='/LISTS_DOCS/goodforHadISDH.'+version+'_IDPHArh.txt'
# Good stations list to carry on for further processing - for Td (derived)
# TDGOTNEIGHBOURSLIST='/LISTS_DOCS/goodforHadISDH.'+version+'_PHADPDtd.txt'
# Plot showing raw and homogenised station with all raw neighbours - abs and anomalies, with linear trends (median pairwise)
# OUTPLOT='/MONTHLIES/HOMOG/STAT_PLOTS/IDADJCOMP/RHDIR/'
# 
# -----------------------
# VERSION/RELEASE NOTES
# -----------------------
#
# Version 5 (25 January 2021)
# ---------
#  
# Enhancements
#  
# Changes
# This is now run from the command line with choice of var and run all or restart from station ??????
# Can also be run from F8_submit_spice.bash
#  
# Bug fix
#
#
...
#  
# Bug fix
# Added extra line for when working on T so that the RAW station when read in for plotting
# is also used in the anomaly plot - it was plotting the PHA version instead.
#
# 
...
INDIRHOM = workingdir+'/MONTHLIES/HOMOG/PHAASCII/' # for t
INDIRRAW = workingdir+'/MONTHLIES/ASCII/' 

# Break locations are taken from pha output
TBREAKFIL   = workingdir+'/LISTS_DOCS/HadISDH.landT.'+versiondots+'_PHA.log'
DPDBREAKFIL = workingdir+'/LISTS_DOCS/HadISDH.landDPD.'+versiondots+'_PHA.log'

#workingdir  = '/scratch/hadkw/UPDATE'+updateyyyy
OUTDIRDAT = workingdir+'/MONTHLIES/HOMOG/IDPHAASCII/' 
OUTDIRPLOT = workingdir+'/MONTHLIES/HOMOG/STAT_PLOTS/IDPHAADJCOMP/' 
...
def ApplyJumpsReZero(TheAnomalies,TheHomogAnoms,TheBreakList,TheBCount,TheActualBreakLocs,TheStClim,TheEdClim,TheMDI,TheMean):
    ...
    climpoints = range(TheStClim,TheEdClim+1)	# range(0,29) only goes from 0 to 28 so need a,b+1
...
def PlotHomogTS(TheFile,TheStation,TheNeighbours,TheHStation,TheMDI,TheStYr,TheYCount,unit,typee):
    ...
    plt.figtext(0.13,0.86,rawstr,color='r',size=16)
    plt.figtext(0.13,0.82,homstr,color='b',size=16)
...
def main(argv):
    ...
    try:
        ...
        print('Usage (as strings) F8_IndirectPHA.py --var <q> --runtype <all>')
        sys.exit(2)
...
#    assert var != '' and typee != '', "Input values not specified."
    ...
    # For T only
    STATSUFFIXINT = '_PHAadj.txt'
    STATSUFFIXOUT = '_IDPHAadj.txt'
 
    STATLIST = INDIRLIST+'goodforHadISDH.'+versiondots+'_IDPHAall.txt'
    CORRFIL = INDIRPHA+var+'/corr/corr.log'

    BREAKSINFO = INDIRLIST+'HadISDH.land'+ParamDict[var][0]+'.'+versiondots+'_IDPHA.log'
    # For T only
    BREAKSINFOMERGE = INDIRLIST+'HadISDH.land'+ParamDict[var][0]+'.'+versiondots+'_IDPHAMG.log'
    NONEIGHBOURSLIST = INDIRLIST+'noneighbours_land'+ParamDict[var][0]+'.'+versiondots+'_IDPHA.txt'
    GOTNEIGHBOURSLIST = INDIRLIST+'goodforHadISDH.'+versiondots+'_IDPHA'+var+'.txt'
    # For T only
    TDGOTNEIGHBOURSLIST = INDIRLIST+'goodforHadISDH.'+versiondots+'_PHADPDtd.txt'
    # For T only
    TDBADSFILE = INDIRPHA+'td/corr/badlist.txt'

    INRAW = INDIRRAW+ParamDict[var][3]+'ABS/'
    # Input PHA homogenised T from PHAASCII
    INHOM = INDIRHOM+'TDIR/'
...
#********************************************
        if var == 't':
            MyFile       = INHOM+StationListWMO[st]+StationListWBAN[st]+STATSUFFIXINT
            print(st,MyFile)  
...
#*********************************************
        ...
        if (var != 't'):	# no need to apply T breaks if working on T 
...
#*****************************************************
            if var == 't':
            # for t need to read in raw rather than PHA homogenised station (which was originally read in to additionally apply DPD break locs)
            ...
            # Plot cand vs neighbours for all vars    
            MyPlotFile = OUTPLOT+StationListWMO[st]+StationListWBAN[st]+'_trendcomp_'+var+'_abs'
            PlotHomogTS(MyPlotFile,MyStation,NeighbourStations,MyHomogAbs,mdi,styr,nyrs,ParamDict[var][1],'absolutes')
            MyPlotFile = OUTPLOT+StationListWMO[st]+StationListWBAN[st]+'_trendcomp_'+var+'_anoms'
            PlotHomogTS(MyPlotFile,MyAnomalies,NeighbourAnomsStations,MyHomogAnoms,mdi,styr,nyrs,ParamDict[var][1],'anomalies')
...
#*******************************************************************
# if var = t then also list station in goodforHadISDH*PHADPDtd list of IDPHAt 
...
#*************************************
# end loop of stations - write out stats to OutPutLog
#************************************************
#    stop()
    # Write out stations with no neighbours from PHA correlations
    os.system("cat "+NONEIGHBOURSLIST+" >> "+OUTPUTLOG) # this appends contents exactly
    
    CountF = int(check_output(["wc","-l",NONEIGHBOURSLIST]).decode('utf-8').split()[0])
    filee = open(OUTPUTLOG,'a+')
    filee.write('%s%s%s%i\n' % ('FAILEDCORR_',ParamDict[var][0],'=',CountF))
    filee.close()
    CountG = int(check_output(["wc","-l",GOTNEIGHBOURSLIST]).decode('utf-8').split()[0])
    filee = open(OUTPUTLOG,'a+')
    filee.write('%s%s%s%i\n' % ('PASSEDCORR_',ParamDict[var][0],'=',CountB))
    filee.close()
        
    if (var == 't'):
    
        CountG = int(check_output(["wc","-l",TDGOTNEIGHBOURSLIST]).decode('utf-8').split()[0])
        filee = open(OUTPUTLOG,'a+')
        filee.write('%s%i\n' % ('PASSEDCORR_Td=',CountB))
        filee.close()
